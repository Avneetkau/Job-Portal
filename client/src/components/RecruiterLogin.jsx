    ...
    <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      <form onSubmit={onSubmitHandler} action ="" className='relative bg-white p-10 rounded-xl text-slate-500'>
          ...
          { state !== 'Login' && (
            <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
             ...
             <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
                ...
                <input className='outline-none text-sm' onChange={e => setEmail(e.target.value)} value={email} type="email" placeholder="Enter Email" required/>
            </div>
             <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
        ...
        <button type='submit' className='bg-blue-600 w-full text-white py-2 rounded-full mt-4'>
            {state === 'Login' ? 'login' : isTextDataSubmitted ?'Create an Account': 'Fill all details'}
        </button> 
         { state === 'Login'
              ?   <p className='mt-5 text-center'>Didn't have an account?<span className='text-blue-600 cursor-pointer' onClick={()=>setState('Sign Up')}>Signup</span></p>
